...
def _flush(self):
    ...
    if not self._pending_events: ## Nada para gravar porém esse atributo debug eu ainda tenho que olhar melhor futuramente
        if getattr(self, "debug", False):
            print("No pending events to flush.")
        return
    else:
        pass
        # print("beginning to flush the events")
    
    with self._buffer_lock:
        events_to_insert = []
        # for eventinho in self._pending_events:
            # print("o ev no pending_events começa assim: ",eventinho)
        ...
        for ev in self._pending_events:
            ...
            if "newCurrentWindow" in ev:
                # print("tem a porra do newCurrentWindow nesse evento!! ", ev)
                try:
                    windowChangeId = _flush_windowChange(self, ev["newCurrentWindow"], ts)
                    # print('the ev["newCurrentWindow"] is: ', ev["newCurrentWindow"] )
                    # print('the type of ev["newCurrentWindow"] is: ', type(ev["newCurrentWindow"]))
                except Exception as e:
                    print("deu merda no flushworker e é: ",e)
                    print("e o ev é: ",ev)
    ...
    for attempt in range(max_retries):
        try:
            # print("about to flush the events!!!!!")
...
def _flush_worker(self):
        ...
        while not self._stop_event.is_set():
            ...
            timeToFlush = False
            with self._buffer_lock:
                if self._pending_events and (time.time() - self._last_flush >= self.flush_interval):
                    timeToFlush = True
